...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const penalityCheck = async (loanDATA) => {
      ...
      let Due_Array = MonthPaying_New_Array;
      ...
      Due_Months_input.addEventListener("input", (e) => {
        ...
        Due_Amount.value = Monthly_Pay * parseInt(Due_Months_input.value);
  ...
  const MonthlyDueArray = () => {
    ...
    let Account_Closed_status = false;
    ...
    Monthly_Array.forEach((loan, ind) => {
      if (Due_Limit_Month < Loan_Months_Count)
        if (loan.paid == false) {
          ...
          if (_Balance_principle == 0 || _Balance_intrest == 0) {
            loan.Account_Closed_status = true;
            Account_Closed_status = true;
          } else {
    ...
    //   if (Monthly_Array[i].paid == false) {
    //     Monthly_Array[i].paid = true;
    //     Monthly_Array[i].Bill_Date = Bill_Date;
    //     Monthly_Array[i].Receipt_Date = Receipt_Date;
    //   }
    // }
    let Penality_Limit_Month = 0;
    Monthly_Array.forEach((loan, ind) => {
  ...
  const UploadLoan = () => {
    ...
    let RC_Array = LoanData_Array.RC_Array;
    let TA_Array = LoanData_Array.TA_Array;

    fetch("/pay-due-current", {
      method: "post",
      headers: {
      ...
      body: JSON.stringify({
        ...
        RC_Array,
        TA_Array,
        AccountStatus: Account_Closed_status,
        _Balance_principle,
        _Balance_intrest,
      }),
    })
    ...
    <div>
      ...
      <div className="main">
        <div className="main-header">
          ...
          <div className="main-title">Automatic Receipt</div>
        </div>
