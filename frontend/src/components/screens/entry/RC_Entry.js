...
const Dashboard = () => {
  ...
  const [Balance_RC_Amount, setBalance_RC_Amount] = useState("");
        ...
        .then((result) => {
          ...
          LoanArray.forEach((loan, index) => {
            ...
            if (loan.LoanObject.LoanNo == loanNO_Input.value) {
              ...
              let Total_RC_Amount = loanData.RC_Amount;
              let balance_RC_Amount = Total_RC_Amount;

              let RC_Array = loanData.RC_Array;
              if (RC_Array.length) {
                balance_RC_Amount = RC_Array[RC_Array.length - 1].Balance;
              }
              setBalance_RC_Amount(balance_RC_Amount);
            }
          });
        });
    });
  ...
  const UploadLoan = () => {
    ...
    let Total_RC_Amount = LoanData_Array.RC_Amount;
    let Balance_RC_Amount = parseInt(Total_RC_Amount);

    let RC_Array = LoanArray.RC_Array;
    ...
    let Current_RC_Balance = Balance_RC_Amount - _Credit_Amount;

    if (Debit_Amount.length) {
      LoanArray.RC_Amount = Balance_RC_Amount + _Debit_Amount;
                    ...
                    <div className="col-12 selfEntry_Container">
                      ...
                      <div class="row" style={{ marginTop: "10px" }}>
                        <input
                          ...
                          value={Balance_RC_Amount}
                          onChange={(e) => setBalance_RC_Amount(e.target.value)}
                        />
                      </div>
                      <div class="row " style={{ marginTop: "10px" }}>
                        <input
